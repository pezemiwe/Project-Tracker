...
import { useSettings, useUpdateSetting, useTestEmail } from "../hooks/useSettings";
...
export default function AdminSettingsPage() {
  ...
  const groupedSettings = useMemo(() => {
    ...
    return settings.reduce((acc, setting) => {
      ...
      let category = "system";
      if (setting.key.toLowerCase().includes("approval")) category = "approval";
      else if (
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-display text-3xl text-foreground mb-2 uppercase tracking-wider">ACCESS DENIED</h1>
          <p className="text-muted-foreground">System settings are restricted to Admin users.</p>
        </div>
      </div>
    );
  }
                ...
                >
                  ...
                  <div className="px-6 py-4 border-b border-border bg-muted">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <Icon className={`w-5 h-5 text-${categoryConfig.color}-400`} />
                      </div>
                  ...
                  <div className="p-6 space-y-6">
                    {categorySettings.map((setting) => {
                      ...
                      const isBoolean = isBooleanField(setting.key, setting.value);
                              ...
                              <div className="flex items-center gap-2">
                                {isBoolean ? (
                                  ...
                                  <div className="relative flex-1 max-w-md">
                                    <input
                                      type={
                                        isPassword && !showPassword[setting.key]
                                          ? "password"
                                          : isNumber
                                          ? "number"
                                          : isEmail
                              ...
                              {setting.updatedAt && (
                                <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span>Last updated: {formatTimestamp(setting.updatedAt)}</span>
                                  </div>
                                  {setting.updatedBy && (
                                    <div className="flex items-center gap-1">
                                      <span>by {setting.updatedBy.fullName}</span>
                                    </div>
                                  )}
                                </div>
                              )}
