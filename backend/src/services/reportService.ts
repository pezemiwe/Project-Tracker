...
export class ReportService {
  ...
  async generateFinancialReport(): Promise<Buffer> {
    ...
    for (const objective of objectives) {
      ...
      doc.fontSize(12).font("Helvetica-Bold").text(`${objective.sn}: ${objective.title}`);
    ...
    doc
      .fontSize(8)
      .font("Helvetica")
      .text("Generated by Donor Oversight System", 50, 750, { align: "center" });
  ...
  async generateAuditReport(filters: {
    ...
    if (filters.objectType)
      doc.text(`Object Type: ${filters.objectType}`);
    if (filters.action) doc.text(`Action: ${filters.action}`);
    if (filters.startDate)
      doc.text(`Start Date: ${new Date(filters.startDate).toLocaleDateString("en-GB")}`);
    if (filters.endDate)
      doc.text(`End Date: ${new Date(filters.endDate).toLocaleDateString("en-GB")}`);
    if (!filters.objectType && !filters.action && !filters.startDate && !filters.endDate) {
  ...
  async generateActivityReport(): Promise<Buffer> {
    ...
    for (const objective of objectives) {
      ...
      if (objective.description) {
        doc.text(objective.description, { width: 500 });
        doc.moveDown(0.5);
      }

      doc.text(`State: ${objective.state || "N/A"}`);
      doc.text(`Sector: ${objective.sector || "N/A"}`);
      doc.moveDown(1);
      ...
      for (const activity of objective.activities) {
        ...
        doc.fontSize(11).font("Helvetica-Bold").text(activity.sn, {
          continued: true,
        });
